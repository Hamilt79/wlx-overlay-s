...
impl SettingType {
	...
	pub fn mut_bool(self, config: &mut GeneralConfig) -> &mut bool {
		match self {
			...
			Self::SpaceGravityFloorEnabled => &mut config.space_gravity_floor_enabled,
	...
	fn get_translation(self) -> Result<&'static str, &'static str> {
		match self {
			...
			Self::SpaceGravityFloorEnabled => Ok("APP_SETTINGS.SPACE_GRAVITY_FLOOR_ENABLED"),
	...
	fn get_tooltip(self) -> Option<&'static str> {
		match self {
			...
			Self::SpaceGravityFloorEnabled => Some("APP_SETTINGS.SPACE_GRAVITY_FLOOR_ENABLED_HELP"),
